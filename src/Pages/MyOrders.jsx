...
const MyOrders = () => {
  ...
  // Filter foods based on the logged-in user's email in the purchases array
  const userFoods = foods.filter((food) =>
    food?.purchases?.some(
      (purchase) =>
        purchase.buyer?.email?.toLowerCase() === user?.email?.toLowerCase()
    )
  );
